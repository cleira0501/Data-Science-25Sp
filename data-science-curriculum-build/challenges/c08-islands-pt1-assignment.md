...
df_q3 <- 
  df_sample_seq %>% 
  mutate(last_name= word(name,-1)) %>% #get last names
  count(last_name, name = "n") %>% # count the last name occurences
  mutate(p = n / nrow(df_sample_seq)) %>%  #get the prevailance
  arrange(desc(p)) #making sure its descending
...
- Is this sample representative of *all* houses in Helvig? Why or why
  not?
  - no because it only represents the first 25 homes in Helvig, people
    who doesn’t live in the first 25 houses don’t get represented.
...
``` r
## NOTE: Do not edit
df_sample_random <- read_csv("./data/helvig-seq-new.csv")
```
...
- Which sample—sequential or random—is more *representative* of all
  homes Helvig? Why?
  - Random
  - Because families are more likely to live closer to eachother, and
    there are “little italy” or “china town” in a lot of the cities we
    see in real life. Since sequential sampling picks homes that are
    near each other, you’re more likely to end up with households that
    have similar socioeconomic backgrounds. That makes the sample less
    diverse. Random sampling spreads out the selection, making it a
...
- Did you find any highly prevalent names using `df_sample_random` that
  ...
  - the top 1 prevalent name is Collins in this question but it is not
    present at all in q3. same with Regan which is the 2nd prevalant
    here
- Is there any reason that people with the same last name might tend to
  *live near each other*?
  - they might be related or share the same ethnicity or culture. It
    makes sense in real life as immigrants form communities to help each
    other from the same country because of language/cultural familiarity
    and trust.
...
name_prevalence <- function(df, last = "Collins") {
  df %>% 
    filter(!is.na(name)) %>%  # Remove invalid names (NA)
    mutate(last_name = word(name, -1)) %>%  # Extract last names
    summarize(estimate = sum(last_name == last) / n()) %>%  # Compute proportion 
    mutate(term = last)
}

name_prevalence(df_sample_random, last = "Collins")
```
...
## HINT: Remember that you need to use analysis() when operating on split_df
        analysis(split_df) %>% 
        name_prevalence(last = "Collins") #"Regan"
      }
    )
  ) %>% 
...
```
    ...
    ## 1 Collins 0.0351     0.107  0.190   0.05 percentile
...
- Note that we used the *random* sample with the bootstrap procedure in
  ...
  - No
  - The bootstrap method relies on the idea that a random sample from
    the population can serve as a good approximation of the population
    itself. However, we have already established in the previous problem
    that sequential sampling is not representative enough of the whole
    population. Therefore, even if you perform the bootstrap method, it
    can still have biases since the original subset already has the
    biases embedded.
...
- I am interested in if having children influences their wealth at
  different ages(maybe they get poorer at first then get wealthier later
  in life, or having 2 children doesn’t actually make you poorer than
  one child)
- I see monetary incentive and I am interested in if they perform a task
  better or worse with or without monetary incentives(there is a tedtalk
  that tell you that it make you perform certain tasks worse)
- I see that you can ask them how they are feeling, I want to measure
  the relationship between happiness and age(research say that you get
  less happy as you age but you get happier after you get old)
- I also see that there are hotels, I’m interested in understanding
  which travel destinations are most popular among different demographic
  groups and where these groups are traveling from
