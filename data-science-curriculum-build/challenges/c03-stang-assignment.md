...
- Is there “one true value” for the material properties of Aluminum?
  - No, there is however a range of values.
- How many aluminum alloys are in this dataset? How do you know?
  - 26, the length of alloy is 26 when you use summary, I also use
    counted to verify
- What angles were tested?
  - 0, 45. 90
- What thicknesses were tested?
  - 0.022, 0.032, 0.064, 0.081 -how does angle affect E and nu -Is there
    any correlation between E and nu?
...
df_q3 <- df_stang_long %>%
  ggplot(aes(nu, E, color = as_factor(angle))) +
  geom_point(size = 3) +
  theme_minimal()
df_q3
```
...
``` r
cor(df_stang_long$E, df_stang_long$nu)
```
...
- from the plot above it seems like angle does not affect E nor nu
  because all the datapoints are scattered around the plot without clear
  clustering. However, I really don’t think we can draw conclusions from
  a dataset with few data points (there are only 7-8 datapoints
  representing the different angle groups)
- from the graph we can observe that there is a general positive
  relationship between nu and E, however, the data is also fairly
  scattered so I used the cor function to compute the numerical
  correlation between nu and E, which gives me a 0.66 in
  correlation(which implies that hey have a positive relationship).
  HOwever the graph is still fairly scattered and with few data points
  so I feel hesitant to draw any solid conclusion from the graph.
...
- Does this graph support or contradict the claim above?
  - It contradicts the claim, in the graph it suggests that there are
    generally positive relationship between E and thickness from
    observation of clustering.
- Is this evidence *conclusive* one way or another?
  - this evidence is not conclusive because although through observation
    we can see that this graph suggest that thickness has a positive
    relationship with the variable E, on the horizontal axis of E= 10500
    lies simultaneously thickness = 0.022, 0.032 and 0.081, we also have
    too little data points to draw solid conclusions from it. We can
    only speculate.
